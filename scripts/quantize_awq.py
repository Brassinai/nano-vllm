#!/usr/bin/env python3
"""Quantize a dense local Hugging Face model into nano-vLLM AWQ format."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Destination directory for the AWQ checkpoint.",
    )
    ...
    parser.add_argument("--clip-steps", type=int, default=10)
    parser.add_argument("--min-clip-ratio", type=float, default=0.5)
    parser.add_argument(
        "--device",
        type=str,
...
def main() -> None:
    ...
    if args.clip_steps <= 0:
        raise SystemExit("--clip-steps must be positive.")
    if not 0 < args.min_clip_ratio <= 1:
        raise SystemExit("--min-clip-ratio must be in the range (0, 1].")
    ...
    config = AWQExportConfig(
        ...
        min_clip_ratio=args.min_clip_ratio,
        dtype=args.dtype,
        device=args.device,
    )
    texts = load_calibration_texts(args)
    result = export_awq_checkpoint(
        args.model,
        output,
        config=config,
